...
<section class="content-header">
    <h1>
        <?= ($a->id == "" ? 'CERTIFICATE OF WORK IN HEIGHT' : 'CERTIFICATE OF WORK IN HEIGHT'); ?> 
    </h1>
    <ol class="breadcrumb">
        <li><a href="<?= $patch ?>">Home</a></li>
        <li><a href="<?= $patch ?>clientes">Ats</a></li>
        <li class="active"> <?= ($a->id == "" ? 'CERTIFICATE OF WORK IN HEIGHT' : 'CERTIFICATE OF WORK IN HEIGHT'); ?> </li>
    </ol>
</section>
<br/>
<div class="box ">
    <form id="form1" class="form" action="<?= $patch; ?>height/save" method="post" name="form1">
        <div class="box-body">
            <fieldset style="width:97%;">
                ...
                <div class="col-lg-4">
                    <label id="l_id_empleado_autoriza">Name of Supervisor:</label>
                    <div class="input-group margin-bottom-20">
                        <span data-toggle="modal"  class="input-group-addon">
                            <i  class="fa fa-code-fork "></i>
                        </span>
                        <select class='form-control' name="id_empleado_autoriza" id="id_empleado_autoriza">
                            <option value="N">[Select]</option>
                            <?php foreach($data["empleados"] as $empleados){?>
                                <option <?php echo  $a->id_empleado_autoriza==$empleados["id"] ? 'selected' : '' ?> value="<?php echo $empleados["id"] ?>"><?php echo $empleados["nombre"]." ".$empleados["apellido"] ?></option>
                            <?php } ?>
                        </select>
                    </div><!-- /.input group -->
                </div>
                ...
                <div class="col-lg-4">
                    <label id="l_hora_inicio">Height*</label>
                    <div class="input-group date">
                        <div class="input-group-addon">
                            <i class="fa fa-code-fork"></i>
                        </div>
                        <input type="text"  value='<?= $a->altura ?>'  class="form-control pull-right" name='altura' id="altura">
                    </div>
                </div>
                <div class="col-lg-4">
...
$('#hora_inicio').datetimepicker({
  ...
  function getAssociate(id){
        $.post("<?= $patch ?>height/getAssociate",{id},function(data){
            $(".html").html(data)
        })
    }
    ...
    $("#btn-save").click(function(){
        ...
        if(validateForm()){
            ...
            if(bad){
               $.post("<?= $patch ?>height/setBadAssociate",
               {
                   ids,
                   id_trabajo:$("#id_trabajo").val(),
                   
                   altura:$("#altura").val(),
                   id_empleado_autoriza:$("#id_empleado_autoriza").val(),
                   id_empleado_autorizado:$("#id_empleado_autorizado").val(),
                   hora_inicio:$("#hora_inicio").val(),
                   hora_final:$("#hora_final").val(),
                   status:'N'
               },function(data){
                    window.location = '<?= $patch; ?>height/sing/'+data;
                }) 
            }else{
                $.post("<?= $patch ?>height/setAssociate",
               {
                   id_trabajo:$("#id_trabajo").val(),
                   altura:$("#altura").val(),
                   id_empleado_autoriza:$("#id_empleado_autoriza").val(),
