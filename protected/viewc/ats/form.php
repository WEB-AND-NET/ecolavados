...
<section class="content-header">
    <h1>
        <?= ($a->id == "" ? 'Ats Register' : 'Pack update'); ?> 
    </h1>
    <ol class="breadcrumb">
        <li><a href="<?= $patch ?>">Home</a></li>
        <li><a href="<?= $patch ?>clientes">Ats</a></li>
        <li class="active"> <?= ($a->id == "" ? 'Ats Register' : 'Ats update'); ?> </li>
    </ol>
</section>
<br/>
<div class="box ">
    <form id="form1" class="form" action="<?= $patch; ?>ats/save" method="post" name="form1">
        <div class="box-body">
...
$('#hora_inicio').datetimepicker({
  ...
  function getAssociate(id){
        $.post("<?= $patch ?>ats/getAssociate",{id},function(data){
            $(".html").html(data)
        })
    }
    ...
    $("#btn-save").click(function(){
        ...
        if(validateForm()){
            ...
            var ids={};
            var bad=false;
            $("div[data-acept='N'][aria-checked='true']").each(function(element,attr){
                ids[attr.dataset.name]=attr.dataset.name.substring(4);
                bad=true;
            }) 
            
            if(bad){
               $.post("<?= $patch ?>ats/setBadAssociate",
               {
                   ids,
                   ...
                   status:'N'
                    ...
                    window.location = '<?= $patch; ?>ats/sing/'+data;
                }) 
            }else{
                $.post("<?= $patch ?>ats/setAssociate",
               {
                   id_trabajo:$("#id_trabajo").val(),
                   id_empleado_autorizado:$("#id_empleado_autorizado").val(),
                   hora_inicio:$("#hora_inicio").val(),
